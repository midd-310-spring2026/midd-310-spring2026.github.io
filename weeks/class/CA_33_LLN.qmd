...
Suppose $X \sim \text{Beta}(`r alpha`, `r beta`)$ and I want to approximate $P(`r a` < X < `r b`)$ (i.e. the shaded area below). I know I can evaluate the function/PDF of $X$ using `pbeta()` or by hand, but suppose I do not have access to `pbeta()` for probabilities.

```{r echo = F, fig.height=4, fig.width=7, warning = F}
...
1.  Sample a point $(x_{j}, y_{j})$ uniformly within the rectangle
2.  Evaluate the function of interest at the sampled $x_{j}$. In this case, the function is the $\text{Beta}(`r alpha`, `r beta`)$ PDF: $$f_{X}(x_{j}) = \frac{\Gamma(`r alpha` + `r beta`)}{\Gamma(`r alpha`)\Gamma(`r beta`)} x_{j}^{`r alpha` - 1}(1 - x_{j})^{`r beta` - 1}$$ Thankfully we can used the `dbeta()` function to do this for us!
3.  Record if $y_{j} < f_{X}(x_{j})$
4.  Repeat!

```{r echo = T, eval = T}
# simulate 5000 points uniformly within bounding rectangle
set.seed(1)
B <- 5000
c <- 4
x <- runif(B, 0.1, 0.2)
...
# proportion of sampled points that fall into shaded region of interest
under <- mean(y < f_x)

# multiply by bounding rectangle area 
mc_approx <- under * c * (0.2 - 0.1)
```
...
# sample uniformly within square on (-r, r)
n <- 10000
x <- runif(n, -r, r)
y <- runif(n, -r, r)
